     ...
     if (_active_region->subdomain_id_inclusion(sub_id)) 
	{
	  ...
	  for (unsigned int qp=0; qp<q_bulk.n_points(); qp++)
	      ...
	      for (unsigned int i=0; i<n_dofs_int_conc;i++) 
		{
	      ...
	      //Real mu_const_lhs = 2.0*R*(delta/eps)*(1.0 - mu_qp);
	      //Real mu_const_rhs = 1.0*R*(delta/std::pow(eps,1))*exp(-mu_qp);
	      //Real mu_const_lhs = (delta/eps)*R;
	      Real mu_const_lhs = 0.0;
	      Real mu_const_rhs = 0.0;

	      Real delta_phidt = (heaviside - heaviside_old)/dt; 
	      
	      Real normal_p  = grad_p_qp*grad_phi_qp;
	      normal_p /= Omega*grad_phi_qp.size_sq(); 
	      normal_p *= heaviside*delta_phidt; 
		...
		for(unsigned int j=0; j<n_dofs_conc; j++) 
		  {
		    Ke(i,j)+= JxW[qp]*( phi_conc[i][qp]*phi_conc[j][qp]*(2.0*heaviside-heaviside_old)
					+ dt*(dphi_conc[i][qp]*(Diff*dphi_conc[j][qp])
					      ...
					      +(mu_const_lhs)*phi_conc[i][qp]*phi_conc[j][qp]
